...
def main():
    ...
    dllogger.init(backends=[dllogger.JSONStreamBackend(verbosity=dllogger.Verbosity.VERBOSE, filename=args.json_summary),dllogger.StdOutBackend(verbosity=dllogger.Verbosity.VERBOSE, step_format=format_step)])
    ...
    if args.do_train:
        ...
        final_loss = None
        dllogger.log(step="PARAMETER", data={"Reached Barrier waiting on others": True})
        torch.distributed.barrier() # Added to sync nodes before training
